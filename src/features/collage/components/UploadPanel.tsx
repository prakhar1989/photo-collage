  ...
  const dropzoneClasses = useMemo(
    () =>
      [
        ...
        isDragging ? 'border-accent bg-accent/5 text-accent' : 'border-border bg-surface text-slate-500',
      ].join(' '),
    [isDragging],
  )
  ...
  return (
    <section className="space-y-4">
      <header className="flex items-start justify-between">
        <div>
          <h2 className="text-base font-semibold text-slate-900">Upload photos</h2>
      ...
      >
        ...
        <div className="space-y-2 text-center">
          <span className="block text-sm font-semibold text-slate-700">Drop photos here</span>
      ...
      {images.length > 0 && (
        <ul className="space-y-2">
          {images.map((image, index) => (
            <li
              key={image.id}
              className="flex items-center gap-3 rounded-xl bg-surface p-2 shadow-sm shadow-slate-100 ring-1 ring-border"
            >
              ...
              <div className="flex-1">
                <p className="truncate text-sm font-medium text-slate-700">{image.file.name}</p>
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  className="rounded-full bg-slate-100 p-2 text-xs text-slate-500 transition hover:bg-slate-200"
                ...
                <button
                  type="button"
                  className="rounded-full bg-slate-100 p-2 text-xs text-red-500 transition hover:bg-red-100"
